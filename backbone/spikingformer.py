...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        self.mlp1_lif = LIFSpike()
        self.mlp1_conv = nn.Conv2d(in_features, hidden_features, kernel_size=1, stride=1)
        self.mlp1_bn = norm_layer(hidden_features)
        self.mlp1 = tdLayer(self.mlp1_conv, self.mlp1_bn)

        self.mlp2_lif = LIFSpike()
        self.mlp2_conv = nn.Conv2d(hidden_features, out_features, kernel_size=1, stride=1)
        self.mlp2_bn = norm_layer(out_features)
        self.mlp2 = tdLayer(self.mlp2_conv, self.mlp2_bn)
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.block3_lif = LIFSpike()
        self.block3_mp = torch.nn.AvgPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=False)
        ...
        self.block4_lif = LIFSpike()
        self.block4_mp = torch.nn.AvgPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=False)
...
@register_model
def Spikingformer(pretrained=False, H=32,W=32,C=3,**kwargs):
    model = vit_snn(img_size_h=H, img_size_w=W, patch_size=16, in_channels=C, num_classes=100,
                 embed_dims=128, num_heads=1, mlp_ratios=4, qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=2, sr_ratios=8, T=4, pretrained_cfg=None,
        **kwargs
    )
